...
#else
            PropertyInfo[] attributedPublicProperties = properties.Where(p => p.GetCustomAttribute(ofAttributeType) != null).ToArray();
#endif
...
#else
            return attributedPublicProperties.Select(pi =>
                new Tuple<PropertyInfo, TAttribType>(pi, pi.GetCustomAttribute(ofAttributeType) as TAttribType)).ToArray();
#endif
        }
    }
}