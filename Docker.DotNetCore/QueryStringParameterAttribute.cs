...
#if NET451
            bool implementsStringConverter = converterType.GetInterfaces().Contains(typeof (IQueryStringConverter));
#else
            bool implementsStringConverter = converterType.GetTypeInfo().ImplementedInterfaces.Contains(typeof(IQueryStringConverter));
#endif

            if (converterType != null && !implementsStringConverter)
            {
